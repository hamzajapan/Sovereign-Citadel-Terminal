...
impl TestHarness {
    ...
    pub fn create_party(&self, name: &str) -> TestParty {
        let (secret, pubkey) = self.secp.generate_keypair(&mut rand::thread_rng());
    ...
    pub fn sign_message(&self, party: &TestParty, message: &[u8]) -> SchnorrSignature {
        ...
        let signature = self.secp.sign_schnorr_no_aux_rand(&msg, &party.keypair);
    ...
    pub fn create_attestation(&self, oracle: &TestParty, value: i64) -> OracleAttestation {
        OracleAttestation {
            ...
            secret: OracleSecret::new([0u8; 32], OracleNonce::from_bytes([0u8; 33])),
